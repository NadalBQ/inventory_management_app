...
def del_item():
    ...
    if not Location:
        ...
        elif int(Amount) == int(df.loc[df['ID'].eq(ID),"Amount"][0]):
            try:
                df = df[~(df['ID'].eq(ID))]
                print(f"Deleted every element with ID={ID}")
                updateDataframe(repository, df, csv)
        ...
        try:
            df.loc[df['ID'].eq(ID),"Amount"] = int(df.loc[df['ID'].eq(ID),"Amount"][0]) - int(Amount)
    ...
    elif int(Amount) == int(df.loc[df['ID'].eq(ID) & df['Location'].eq(Location),"Amount"][0]):
            try:
                df = df[~(df['ID'].eq(ID) & df['Location'].eq(Location))]
                print(f"Deleted every element with ID={ID} and Location={Location}")
                updateDataframe(repository, df, csv)
    ...
    #If User provides ID, Location and Amount of element:
        ...
        try:
            df.loc[df['ID'].eq(ID) & df['Location'].eq(Location),"Amount"] = int(df.loc[df['ID'].eq(ID) & df['Location'].eq(Location),"Amount"][0]) - int(Amount)
